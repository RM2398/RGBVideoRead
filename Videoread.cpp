...
void info(void) {
    std::cout << "EML 6805 Robot Design" << std::endl
              ...
              << "Rashaan Miller <rmill153@fiu.edu>" << std::endl
...
int main(int argc, char** argv) {
    ...
    while(running) {
        ...
        for (int x = 0; x < image.cols; ++x ) {
            ...
            if( pixel[0] > threshold && pixel[1] < threshold && pixel[2] < threshold && !startoftape)
            {
            ...
            if( pixel[0] < threshold && startoftape && !endoftape)
            {
                cv::circle(image, cv::Point(x, y), 5, cv::Scalar(255, 255, 255), 1);
                endoftape = true;
            ...
            cv::Point pb(ptmid, y-10); // bottom point for center line
            cv::Point pt(ptmid,y+10); // top point for center line
